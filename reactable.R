...
# b <- ymd(a$Comunicacao)
a$Dif <- ifelse(difftime(Sys.Date(),a$Comunicacao)>120,"Sim","Não")
a <- a[,c(1,2,3,4,8,9,5,10,6,7)]
library(reactable)
library(htmlwidgets)
reactable(a, compact=TRUE, filterable=FALSE, searchable = TRUE, defaultPageSize = 30,highlight = TRUE,defaultSortOrder="desc",defaultSorted="Atraso",
